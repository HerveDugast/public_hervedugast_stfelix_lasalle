...
global CAPTEUR_ID_TEMP_BMP280 = 1
global CAPTEUR_ID_PRES_BMP280 = 3
...
def sqlInsert2Data(sqlQuery, champ1, champ2):
    """ Exécute une requête SQL de type INSERT, insère 2 données dans la base de données MySQL"""
    args = [champ1, champ2]
    try:
        bddConnect()
        cur = cnx.cursor()
        sql = """INSERT INTO releve_capteur (date_heure, valeur) VALUES (%s, '%s')"""
...
#    threading.Timer(tempo * 60, t1mnTemperature, [tempo]).start() 
    threading.Timer(tempo * 3, t1mnTemperature, [tempo]).start() 
    ## Reste du traitement 
    ...
    sqlQuery = "INSERT INTO releve_capteur (date_heure, valeur) VALUES (%s, '%s')"
    sqlInsert2Data(sqlQuery, timeReleve, mesTemperature)

#    sqlSelectFetchall("SELECT date_heure, valeur FROM releve_capteur")


def t1mnPression(tempo): 
    threading.Timer(tempo * 5, t1mnPression, [tempo]).start() 
    ## Reste du traitement 
    timeCourant = time.strftime('%A %d/%m/%Y %H:%M:%S')
