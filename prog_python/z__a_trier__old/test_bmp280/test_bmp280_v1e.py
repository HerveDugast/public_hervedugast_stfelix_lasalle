...
##            print("Connectée à une base de données MySQL")
        ...
        print("!!!!!!!!!! Error mysql.connector bddConnect", err)
    finally:
        return cnx
...
##            print('last insert id', cur.lastrowid)
        ...
        print("!!!!!!!!!! Error mysql.connector", err) 
    finally:
        cur.close()
        bddClose(cnx)
...
def threadI2c(tempo): 
    """ thread lancé automatiquement toutes les "tempo" minutes
    Effectue les mesures de température et pression atmosphérique """
    threading.Timer(tempo * 60, threadI2c, [tempo]).start() 
    ## Reste du traitement
    # initialisation variable statique
    threadI2c.counter = getattr(threadI2c, 'counter', 100)
    threadI2c.counter += 1
    
    # mesure température (plus fréquente que mesure pression atmosphérique)
    ...
    try:
        ...
        print("!!!!!!!!!! OSError mesTemperature", err)
    except UnboundLocalError as err :
        print("!!!!!!!!!! UnboundLocalError mesTemperature", err)
    except:
        print("!!!!!!!!!! Error unknown")

    # mesure pression atmosphérique (plus fréquente que mesure température)
    if threadI2c.counter >= (INTERVALLE_MESURE_I2C_PRESSION / INTERVALLE_MESURE_I2C_TEMPERATURE):
        try:
            mesPression = int(sensor.read_pressure() / 100)
            print(timeCourant + '  T = ' + str(mesPression) + "mbar")
            sqlQuery = "INSERT INTO releve_capteur (date_heure, valeur, capteur_id) VALUES (%s, '%s', '%s')"
            sqlInsert3Data(sqlQuery, timeReleve, mesPression, CAPTEUR_ID_PRES_BMP280)
            threadI2c.counter = 0
        except OSError as err :
            print("!!!!!!!!!! OSError mesPression", err)
        except UnboundLocalError as err :
            print("!!!!!!!!!! UnboundLocalError mesPression", err)
        except:
            print("!!!!!!!!!! Error unknown mesPression")
