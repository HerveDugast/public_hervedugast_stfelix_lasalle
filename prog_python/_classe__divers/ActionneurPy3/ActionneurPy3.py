...
# coding: utf-8
"""
Classe : ActionneurPy3.py     version : 1.0
Auteur : H. Dugast
Date : 09-01-2017
...
class ActionneurPy3:
    ...
    (LED, relais...) connecté à un port GPIO d'une carte raspberry
    ...
    def __init__(self, pinActionneur = 0xFF, niveauActif = 1):
        ...
        GPIO.setup(self.m_pinActionneur, GPIO.OUT)
        # désactive l'actionneur
        GPIO.setup(self.m_pinActionneur, not self.m_niveauActif)
    ...
    def isActif(self):
        """
        Lit l'état de l'actionneur puis signale s'il est actif.
        Retour :
            True -> actionneur actif
            False -> actionneur inactif
        """
        self.m_niveauActuel = GPIO.input(self.m_pinActionneur)
    ...
    def activer(self):
        ...
        """
        GPIO.output(self.m_pinActionneur, self.m_niveauActif)
        self.m_isActionneurActif = self.s_ACTIF         # Actionneur activé
    ...
    def desactiver(self):
        ...
        Les attributs m_isActionneurActif et m_niveauActuel sont mis à jour
        """
        GPIO.output(self.m_pinActionneur, not self.m_niveauActif)
        self.m_isActionneurActif = not self.s_ACTIF     # Actionneur désactivé
        self.m_niveauActuel = not self.m_niveauActif
    ...
    def changerEtat(self):
        ...
        Les attributs m_isActionneurActif et m_niveauActuel sont mis à jour
        """
        self.m_niveauActuel = not GPIO.input(self.m_pinActionneur)
        GPIO.output(self.m_pinActionneur, self.m_niveauActuel)
        if self.m_niveauActuel == self.s_ACTIF:
            self.m_isActionneurActif = self.s_ACTIF     # Actionneur activé
        else:
            self.m_isActionneurActif = not self.s_ACTIF # Actionneur désactivé
