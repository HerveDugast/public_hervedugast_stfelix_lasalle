...
# coding: utf-8
"""
Programme (classe) : CCompteEpargne.py       version 1.4
Date : 13-04-2018
Auteur : Hervé Dugast
...
class CCompteEpargne(CCompteBanque):
   ...
   def retirerArgent(self, somme=-1):
      ...
      if self.get_solde() - somme >= self.__soldeMinimal:
         # Le solde du compte doit être supérieur ou égal au découvert autorisé après le retrait
         self.set_solde(self.get_solde()-somme, self.__soldeMinimal)
         codeResult = 0
      else:
   ...
   def __afficherResultatOperation(self, codeResult, *args):
      ...
      """
      # ----- erreur 4x ----- : erreurs liés au module CCompteCheque
      # erreur 43 : args[1] -> typeCompte
      if codeResult == 43:
