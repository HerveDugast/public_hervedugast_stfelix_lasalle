...
ETAT_INCONNU = "Inconnu"   # contact dans un état inconnu
ETAT_ON = "Fermé"        # contact fermé
ETAT_OFF = "Ouvert"  # contact ouvert
...
IDX_VAL_MAX = 5   # nb de valeurs du dictionnaire à 2 dimensions, on reprend à 1 après VAL_MAX
idxA518mes = 0
idxA518bdd = 0
...
def thread_captTempIn518(tempo): 
   ...
   try:
      verrou.acquire()  # verrouille l'accès à la ressource
      ...
      verrou.release()  # déverrouille l'accès à la ressource
...
def thread_contactA_518(tempo): 
   ...
   if errorMsg != "OK":
      logger.error(errorMsg)
   # signale tout changement d'état pour traitement
   elif captContA518[idxA518mes + 1]["etatNow"] != captContA518[idxA518mes]["etatPrec"]:
      captContA518[idxA518mes + 1]["etatPrec"] = captContA518[idxA518mes + 1]["etatNow"]
      idxA518mes = idxA518mes + 1
      if idxA518mes == IDX_VAL_MAX + 1:
         idxA518mes = 0
         captContA518[0]["etatPrec"] = captContA518[idxA518mes]["etatPrec"]

   print('------')
   print(idxA518mes, captContA518)
##   heureFinThread = datetime.now()
##   duree = heureFinThread - heureDebutThread
##   print("durée thread Porte = ", duree)

def contactA_518_gerer():
    global captContA518

    timeCourant = time.strftime('%A %d/%m/%Y %H:%M:%S')
    if captContA518["etatNow"] == ETAT_ON:
       print(timeCourant,'  Contact 1-518 : ON')
    else:
       print(timeCourant,'  Contact 1-518 : OFF')
...
thread_contactA_518(3) # lecture contact toutes les x secondes (0.01 -> 10 ms)
...
while True:
   pass
##   if captContA518["isContactChange"]:
##      contactA_518_gerer()
##      captContA518["isContactChange"] = False
