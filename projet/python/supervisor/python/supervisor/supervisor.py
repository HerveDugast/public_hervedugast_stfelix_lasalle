...
# coding: utf-8
"""
Programme : supervis.py     version : 1.5
Auteur : H. Dugast
Date : 15-05-2017
...
isCaptBmp280ready = False
# Variables capteur température et pression (BMP280) -----
captTempIn518 = {}
captTempIn518["nom"] = "Temperat_in_518"   # capteur de température
captTempIn518["corr"] = -2  # correctif, ce capteur retourne une valeur très souvent 1°C au-dessus de la valeur réelle
...
captRaspA518 = {}
captRaspA518["nom"] = "RaspA518"   # carte raspberry n°1 de la salle 518
...
FILE_ERR_NAME = 'error.log' 
FILE_ERR__LEVEL = logging.WARNING  # niveau possible : CRITICAL > ERROR > WARNING > INFO > DEBUG
FILE_ERR_FORMAT = '%(asctime)s :: %(levelname)s :: %(message)s'
FILE_ERR_DATE_FORMAT = '%d-%m-%Y %H:%M:%S'
FILE_ERR_SIZE = 1000000    # taille en octets du fichier log
# nombre de fichiers backups créés lorsque le fichier log est "plein" avec extension log.1,log.2 ...
FILE_ERR_BACKUP_NB = 10     # nombre de backups du fichier log
FILE_ERR_ENCODING = 'utf8'
# création d'un formateur qui va ajouter le temps, le niveau de chaque message quand on écrira un 
# message dans le log
formatter = logging.Formatter(FILE_ERR_FORMAT, FILE_ERR_DATE_FORMAT)
...
file_handler = RotatingFileHandler(FILE_ERR_NAME, 'a', FILE_ERR_SIZE, FILE_ERR_BACKUP_NB, 
                                   FILE_ERR_ENCODING)
# ajout de cet handler au logger pour écrire les informations dans le fichier log 
file_handler.setLevel(FILE_ERR__LEVEL)
file_handler.setFormatter(formatter)
logger.addHandler(file_handler)

# ----- fichier log : suivi système -----
# pour mémoriser tous les événements (mesures, erreurs...) avec handler RotatingFileHandler
FILE_SUP_NAME = 'supervision.log' 
FILE_SUP__LEVEL = logging.INFO  # niveau possible : CRITICAL > ERROR > WARNING > INFO > DEBUG
FILE_SUP_FORMAT = '%(asctime)s :: %(levelname)s :: %(message)s'
FILE_SUP_DATE_FORMAT = '%d-%m-%Y %H:%M:%S'
FILE_SUP_SIZE = 1000000    # taille en octets du fichier log
# nombre de fichiers backups créés lorsque le fichier log est "plein" avec extension log.1,log.2 ...
FILE_SUP_BACKUP_NB = 100     # nombre de backups du fichier log
FILE_SUP_ENCODING = 'utf8'
# création d'un formateur qui va ajouter le temps, le niveau de chaque message quand on écrira un 
# message dans le log
formatter = logging.Formatter(FILE_SUP_FORMAT, FILE_SUP_DATE_FORMAT)
# création d'un handler qui va rediriger une écriture du log vers un fichier en mode 'append',
# et définition du nombre de backups et de la taille du fichier
file_handler = RotatingFileHandler(FILE_SUP_NAME, 'a', FILE_SUP_SIZE, FILE_SUP_BACKUP_NB, 
                                   FILE_SUP_ENCODING)
# ajout de cet handler au logger pour écrire les informations dans le fichier log 
file_handler.setLevel(FILE_SUP__LEVEL)
file_handler.setFormatter(formatter)
logger.addHandler(file_handler)
...
def thread_captTempIn518(tempo): 
   ...
   if isCaptBmp280ready:
      try:
         ...
         mesTemperatureLog = captTempIn518["nom"] + ' = ' + str(mesTemperature / 10) + " °C"
         logger.info(mesTemperatureLog)
      ...
      errorMsg = "OSError capteur BMP280 (Température)"

   if errorMsg != "OK":
      logger.error(errorMsg)
      journal_ecrire_bdd(dateHeureMes, 'Error', captTempIn518["nom"], errorMsg)
...
def thread_captPres518(tempo):
   ...
   if isCaptBmp280ready:
      try:
         ...
         mesPressionLog = captPres518["nom"] + " = " + str(mesPression) + " mbar"
         logger.info(mesPressionLog)
         verrouBmp280.release()  # déverrouille l'accès à la ressource
      ...
      errorMsg = "OSError capteur BMP280 (Pression)"

   if errorMsg != "OK":
      logger.error(errorMsg)
      journal_ecrire_bdd(dateHeureMes, 'Error', captPres518["nom"], errorMsg)
...
def thread_contactA_518(tempo): 
   ...
   ## Reste du traitement
   global captContA518, idxA518mes
   errorMsg = "OK"
   UNE_HEURE = 10
   
   # initialisation variable statique
   thread_contactA_518.errorNumPrec = getattr(thread_contactA_518, 'errorNumPrec', 0)
   thread_contactA_518.errorCpt = getattr(thread_contactA_518, 'errorCpt', UNE_HEURE / tempo)
   ...
   if codeContact > CODE_MIN_ON:
      ...
      # système de détection du nb d'erreurs consécutives pour éviter trop de journalisation
      # UNE_HEURE : 1 journalisation de l'erreur toutes les heures
      errorNum = 1
      if errorNum == thread_contactA_518.errorNumPrec:
         thread_contactA_518.errorCpt += 1
         if thread_contactA_518.errorCpt > UNE_HEURE / tempo:
            thread_contactA_518.errorCpt = 0
            logger.error(errorMsg)
            journal_ecrire_bdd(dateHeureMes, 'Error', captContA518["nom"], errorMsg)
   thread_contactA_518.errorNumPrec = errorNum
...
def contactA_518_memoriser_bdd():
   global captContA518, idxA518bdd
   errorMsg = "OK"
   ...
   if releve["valeur"] == ETAT_ON:
      msgContact = ' : Porte 1-518 fermée ... contact 1-518 = '
      msgEtat = ' (ON)'
   else:
      msgContact = ' : Porte 1-518 ouverte ... contact 1-518 = '
      msgEtat = ' (OFF)'

   #print(releve["date_heure"], msgContact, releve["valeur"], msgEtat)
   mesContactLog = captContA518["nom"] + msgContact + str(releve["valeur"]) + msgEtat
   logger.info(mesContactLog)
...
def bdd_recuperer_periode_mesure():
   ...
   global captTempIn518, captPres518
   errorMsg = "OK"
...
def journal_ecrire_bdd(dateHeure, niveauAlerte, capteurNom, message):
   ...
   journal["capteur_id"] = bdd_requete_retour_1_valeur("id", "capteur", "nom", capteurNom)
...
bdd_verifierAcces()
if bdd.m_messageError == "OK":
   journal_ecrire_bdd(dateHeure, 'Warning', captRaspA518["nom"], message )
   bdd_recuperer_periode_mesure()
   bdd_verifier_existence_detecteur_porte()

   # ----- Création objet capteur BMP280 (Température et pression atmosphérique)
   bmp280_creerObjet()
   # la 1ère lecture du capteur est régulièrement erronée
   mesPression = int(sensor.read_pressure() / 100)
   mesTemperature = int( (sensor.read_temperature() + captTempIn518["corr"]) * 10)
   ...
   logger.error("Impossible de se connecter à la base de données... Fin du programme")
