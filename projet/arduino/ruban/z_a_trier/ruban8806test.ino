...
#define BASE_TEMPS   500           // base de temps en millisecondes
...
// ------------- ruban à LED ----------------------------------------------------------------------
const uint8_t SEQUENCE_A_TESTER = 6;  // choix entre séquence num 0 à 6      9 -> toutes les seq
const uint16_t NB_LED_RUBAN = 64;	// nombre de LED dans le ruban
...
void loop()
{
   ...
   if (millisNow - millisPrec >= BASE_TEMPS)
   {
      ...
      case 0:  // allumer tout le ruban en rouge
         ruban.eteindre();
         ruban.preparerCouleurIntensite(1, NB_LED_RUBAN, Ruban8806::s_COUL_BLANC, 100);
         ruban.allumer(); break;
      ...
      case 4:  // allumer les LED proportionnellement à la position du curseur du potentiomètre
         ...
         ruban.eteindre();
         ruban.preparerCouleurIntensite(0, ruban.get_nbLedOn(), ruban.s_COUL_BLANC, 100);
         ruban.allumer(); break;
      case 5:  // allumer les LED avec un dégradé d'une couleur vers une autre couleur
         ruban.eteindre();
         ruban.preparerCouleurDegrade(ruban.s_COUL_VERT, ruban.s_COUL_ROUGE, 40, NB_LED_RUBAN);
         ruban.allumer(); break;
